...
module Gist
  ...
  def execute(*args)
    ...
    begin
      ...
      url = write(files, private_gist, description, update_to, anonymous_gist)
  ...
  def write(files, private_gist = false, description = nil, update_to=nil, anonymous_gist = false)
    access_token = (File.read(File.expand_path(CONFIG_FILE)) rescue nil)

    url = CREATE_URL
    if update_to
      url << "/#{update_to}"
    elsif access_token.to_s != '' && !anonymous_gist
